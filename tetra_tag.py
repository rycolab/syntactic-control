...
class TetraTagSystem:
    ...
    def __init__(self, tag_vocab=None, trees=None):
        ...
        self._leaf_tags_only = np.asarray(-1e9 * (1 - is_leaf_mask), dtype=float)
    ...
    def ids_from_tree(self, tree, ignore_unknown=True):
        ...
        if not ignore_unknown:
            return [self.tag_vocab.index(tag) for tag in self.tags_from_tree(tree)]
        return [
            (
    ...
    def ids_from_logits(self, logits, mask=None):
        ...
        for t in range(logits.shape[0]):
            ...
            if last_t is not None:
                beam_search.advance(logits[last_t, :] + self._internal_tags_only)
...
def tree_to_tags_helper(
    ...
    if not isinstance(tree, nltk.Tree):
        tag = "r" if is_right_branch else "l"
        return [], [tag]
...
class BeamSearch:
    ...
    def advance(self, label_logits):
        ...
        all_new_stack_depths = (
            self.beam.stack_depths[:, None] + self.stack_depth_change_by_id[None, :]
        )
